...
export function ComboboxTrigger({ children, className }: ComboboxTriggerProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative z-20 flex h-9 w-full min-w-52 cursor-text items-center justify-between gap-2 rounded-md border border-border bg-transparent px-3 text-sm text-foreground transition-[border-color] hover:border-(--color-border-strong)",
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className={cn(
          "h-8 min-w-0 flex-1 bg-transparent text-sm text-foreground outline-none placeholder:text-muted-foreground disabled:cursor-not-allowed",
          className,
        )}
      />
    </div>
