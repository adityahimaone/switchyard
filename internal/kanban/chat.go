...
func ensureChatDB() (*sql.DB, error) {
	...
	stmts := []string{
		`CREATE TABLE IF NOT EXISTS chat_sessions (id TEXT PRIMARY KEY, title TEXT NOT NULL, agent TEXT NOT NULL DEFAULT 'hermes', profile TEXT NOT NULL DEFAULT 'default', workspace TEXT NOT NULL DEFAULT '', model TEXT NOT NULL DEFAULT '', created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, archived INTEGER NOT NULL DEFAULT 0)`,
...
func ListChatSessions() ([]ChatSession, error) {
	...
	defer db.Close()
	rows, err := db.Query(`SELECT id,title,agent,profile,workspace,model,created_at,updated_at FROM chat_sessions WHERE archived=0 ORDER BY updated_at DESC, created_at DESC LIMIT 200`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var s ChatSession
		if err := rows.Scan(&s.ID, &s.Title, &s.Agent, &s.Profile, &s.Workspace, &s.Model, &s.CreatedAt, &s.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, s)
	}
...
func GetChatSession(id string) (*ChatSession, error) {
	...
	defer db.Close()
	var s ChatSession
	if err := db.QueryRow(`SELECT id,title,agent,profile,workspace,model,created_at,updated_at FROM chat_sessions WHERE id=?`, id).Scan(&s.ID, &s.Title, &s.Agent, &s.Profile, &s.Workspace, &s.Model, &s.CreatedAt, &s.UpdatedAt); err != nil {
		return nil, err
	}
	return &s, nil
}
...
func UpdateChatSession(id string, title, agent, profile, workspace, model *string) (*ChatSession, error) {
	...
	defer db.Close()
	var cur ChatSession
	if err := db.QueryRow(`SELECT id,title,agent,profile,workspace,model,created_at,updated_at FROM chat_sessions WHERE id=?`, id).Scan(&cur.ID, &cur.Title, &cur.Agent, &cur.Profile, &cur.Workspace, &cur.Model, &cur.CreatedAt, &cur.UpdatedAt); err != nil {
		return nil, err
	}
	if title != nil {
...
func appendChatRunEventLocked(db *sql.DB, runID, kind, payload string) error {
	...
	broadcastEvent("chat_run_event", map[string]any{"run_id": runID, "kind": kind})
	return nil
}
