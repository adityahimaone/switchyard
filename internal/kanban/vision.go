...
func AnalyzeAttachment(ctx context.Context, id, model, prompt string) (string, error) {
	...
	if err != nil { return "", err }
	if !CanAnalyze(model, a.MIME) { return "", fmt.Errorf("model %q cannot analyze %s", model, a.MIME) }
	cfg, err := loadVisionProvider()
	if err != nil { return "", err }
	if cfg.BaseURL == "" || cfg.APIKey == "" { return "", fmt.Errorf("OpenAI-compatible provider missing base_url or api_key") }
	if prompt == "" { prompt = "Analyze this attachment. Describe important content, extract actionable facts, and flag uncertainty." }
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		if out.Error != nil && out.Error.Message != "" { return "", fmt.Errorf("vision provider: %s", out.Error.Message) }
		return "", fmt.Errorf("vision provider returned HTTP %d", resp.StatusCode)
	}
	if len(out.Choices) == 0 || strings.TrimSpace(out.Choices[0].Message.Content) == "" { return "", fmt.Errorf("vision provider returned empty response") }
	return strings.TrimSpace(out.Choices[0].Message.Content), nil
}

func loadVisionProvider() (visionProviderRaw, error) {
	raw, err := os.ReadFile(hermesHome() + "/config.yaml" )
	if err != nil { return visionProviderRaw{}, err }
	var cfg visionConfigRaw
	if err := yaml.Unmarshal(raw, &cfg); err != nil { return visionProviderRaw{}, err }
	for _, p := range cfg.CustomProviders {
		for id := range p.Models {
			if id == "" { continue }
			// Model roster can contain aliases; select provider owning requested model later.
		}
		if strings.Contains(strings.ToLower(p.Name), "9router") { return p, nil }
	}
	for _, p := range cfg.CustomProviders { if p.BaseURL != "" && p.APIKey != "" { return p, nil } }
